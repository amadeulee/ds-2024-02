...
public class TableServiceServiceV2 {
    ...
    public void createTableService(AddTableServiceRequest addTableServiceRequest) {
        ...
        var employee = userRepository.findByIdAndIsDeletedFalse(addTableServiceRequest.waiterId())
                .orElseThrow(() -> new BadRequestException("Employee not found"));

        valida(employee, table);
    ...
    private static void valida(User employee, CustomerTable table) {
        if(!isWaiter(employee)) {
    ...
    private static boolean isWaiter(User employee) {
        return employee.getRole().equals(UserRoleEnum.WAITER);
    }
